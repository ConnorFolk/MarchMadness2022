...
t=confusionMatrix(factor(test$Result), ML_trainphatsxgb, threshold = .5)
coftable=function(confmat){
options(knitr.kable.NA = '')
XGboostCM=as.data.frame(confmat)
XGboostCM[3,]= c(NA, paste0(round((XGboostCM[1,1] + XGboostCM[2,2])/ sum(XGboostCM) * 100,2), '%'))
colnames(XGboostCM)= c("Predicted Team 2 Won", "Predicted Team 1 Won")
row.names(XGboostCM)=c("Actual Team 2 Won", "Actual Team 1 Won", "Classification Rate")

return(kable(XGboostCM)%>%
...
coftable(confusionMatrix(factor(test$Result), rfphat, threshold = .5))
