...
RegSeason=read_csv("MDataFiles_Stage2/MRegularSeasonDetailedResults.csv")
...
NCAATourneySched=read_csv("MDataFiles_Stage2/MNCAATourneyCompactResults.csv")
NCAASEEDS= read.csv("MDataFiles_Stage2/MNCAATourneySeeds.csv")
Massey= read.csv("MDataFiles_Stage2/MMasseyOrdinals_thruDay128.csv")

#If team 1 wins then it is a one
NCAASCHED=rbind(NCAATourneySched%>%
  rowid_to_column('ID')%>%
  ...
  NCAATourneySched%>%
    rowid_to_column('ID')%>%
    mutate(Team2= ifelse(ID %% 2 != 0, WTeamID, LTeamID),
           Team1= ifelse(ID %% 2 != 0, LTeamID, WTeamID),
           Result= ifelse(ID %% 2 != 0, 0, 1))%>%
    select(Season, Team1,Team2, Result)%>%
    filter(Season >=2003))
